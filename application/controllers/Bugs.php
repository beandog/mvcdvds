<?php
	...
	class Bugs extends CI_Controller {
		...
		function update_dvd($dvd_id) {
			...
			$arr_bugs = $this->dvds_model->get_bugs($dvd_id, null);

			$post = $this->input->post();

			$array_keys = array();

			if(array_key_exists('dvd_bug', $post))
				$array_keys = array_keys($post['dvd_bug']);

			foreach($arr_bugs as $arr_bug) {
			...
			$this->dvds_model->set_notes($post['notes']);
